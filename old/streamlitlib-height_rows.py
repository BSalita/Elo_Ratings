...
def ShowDataFrameTable(table_df,key=None,output_method='aggrid',color_column=None,ngroup_name=None,round=2,tooltips=None,height_rows=None):
    ...
    if output_method == 'table':
        ...
        # Compute dynamic height based on actual dataframe size
        if height_rows is None:
            rows_to_show = len(table_df) if len(table_df) > 0 else 1
        else:
            rows_to_show = min(int(height_rows), len(table_df)) if len(table_df) > 0 else 1
        # Calculate tight height: header (40px) + rows (25px each) + minimal padding
        height_px = 40 + (25 * rows_to_show) + 5
        st.dataframe(
            table_df.style.format({col:'{:,.2f}' for col in table_df.select_dtypes('float')}).set_table_styles(style_table()),
            width=1600,
            height=height_px
        )
        ...
        if height_rows is None:
            rows_to_show = min(4, len(table_df)) if len(table_df) > 0 else 1
        else:
            rows_to_show = 1 if len(table_df) == 0 else min(int(height_rows), len(table_df))
        # Expand to fit when showing all available rows
...
def dataframe_to_table(df, max_rows: int | None = None, max_cols: int | None = None, shrink_to_fit: bool = False):
    ...
    print(f"DEBUG dataframe_to_table called: shrink_to_fit={shrink_to_fit}, df.shape={df.shape if hasattr(df, 'shape') else 'unknown'}")
    
    # Also try writing to a file in case console output is suppressed
    try:
        with open('pdf_debug.txt', 'a') as f:
            import datetime
            f.write(f"{datetime.datetime.now()}: shrink_to_fit={shrink_to_fit}\n")
    except:
        pass
    ...
    # Store original header texts for checking column types (before wrapping)
    original_headers = []
    ...
    for row_idx, row in enumerate(soup.table.findAll('tr')):
        ...
        for cell in row.findAll(['td', 'th']):
            ...
            # Apply column name wrapping to header row (first row)
            if row_idx == 0:
                original_headers.append(cell_text)  # Store original before wrapping
                # Use shorter max_length for narrower columns when shrink_to_fit is enabled
                max_len = 8 if shrink_to_fit else 12
                wrapped_text = wrap_column_name(cell_text, max_length=max_len)
                # Debug: Show first 5 headers and write to file
                if len(original_headers) <= 5 and shrink_to_fit:
                    try:
                        with open('header_debug.txt', 'a') as f:
                            f.write(f"Header #{len(original_headers)}: '{cell_text}' -> '{repr(wrapped_text)}'\n")
                    except:
                        pass
                cell_text = wrapped_text
                # Plain strings with \n work perfectly for multi-row headers (tested!)
    ...
    if shrink_to_fit and len(table_data) > 0:
        ...
        if col_count > 0:
            ...
            # Use original_headers (before Paragraph conversion) for the check
            pair_names_idx = None
            for i, header in enumerate(original_headers):
                if 'Pair_Names' in header:
                    pair_names_idx = i
                    break
    ...
    table_style = [
        ...
        ('ROWBACKGROUNDS', (0, 0), (-1, 0), ['#E5E5E5']),  # Ensure header background
    ]
    ...
    for a in pdf_assets:
        ...
        # Convert each DataFrame in the list to a reportlab table and add it to the story
            ...
            for i, part in enumerate(frames):
                ...
                print(f"DEBUG create_pdf: About to call dataframe_to_table with shrink_to_fit={shrink_to_fit}, part.shape={part.shape}")
