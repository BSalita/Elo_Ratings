# FFBridge Classic API Documentation

**Base URL:** `https://api.ffbridge.fr/api/v1`  
**Authentication:** Bearer token in header: `Authorization: Bearer {token}`  
**Used by:** `elo_ffbridge_classic.py` adapter

> **Note:** This is the "new" authenticated FFBridge API. For the legacy Lancelot API (public access), see `FFBRIDGE_LANCELOT_API.md`.
...
| `ranking` | Club rank (without handicap) |
| `theoretical_ranking` | Handicapped rank (national ranking) |
| `percent` | Percentage score (**appears to be handicap-adjusted in practice**) |
| `PE`, `PE_bonus` | Performance points (bonus as string, e.g., "10") |
...
- The `percent` field appears to already be the handicap-adjusted score in most events
- To derive club percentage: `club_pct = percent - (PE_bonus / 10.0)`
- `theoretical_ranking` provides the handicapped national ranking
- `ranking` provides the club-level ranking (without handicap)
...
The Classic API is used through the `elo_ffbridge_classic.py` adapter module.

**Key functions:**
```python
import elo_ffbridge_classic as api

# Check authentication
if api.is_authenticated():
    # Fetch all tournaments
    tournaments = api.fetch_tournament_list(series_id="all")
    ...
    # Get player profile with IV rating
    member = api.fetch_member_details(person_id="597539")
```
