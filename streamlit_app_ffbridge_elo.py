...
def main():
    ...
    with st.sidebar:
        ...
        if "selected_api" not in st.session_state:
            st.session_state.selected_api = "FFBridge (Lancelot)"
    ...
    if not results_df.is_empty():
        players_df = duckdb.sql("""
            WITH player_results AS (
                SELECT player1_id AS player_id, player1_name AS player_name, player1_elo_after AS elo_rating, percentage
                FROM results_df
                UNION ALL
                SELECT player2_id AS player_id, player2_name AS player_name, player2_elo_after AS elo_rating, percentage
                FROM results_df
            )
            SELECT 
                player_id,
